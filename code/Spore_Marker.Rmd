...
```{r, results="hide"}
head(UBA_RIBOS)
colnames(UBA_RIBOS) <- c("target name","accession","query name",
  "accession","E-value","score","bias","E-value","score","bias",
  "exp","reg","clu","ov","env", "dom","rep","inc")
colnames(REF_RIBOS) <- c("target name","accession","query name",
  "accession","E-value","score","bias","E-value","score","bias",
  "exp","reg","clu","ov","env", "dom","rep","inc")
table(REF_RIBOS$`query name`)
ggplot(REF_RIBOS, aes(x=score)) + geom_density() + facet_wrap(~`query name`)
REF_RIBOS <- REF_RIBOS[REF_RIBOS$score >= 50,]
ggplot(UBA_RIBOS, aes(x=score)) + geom_density() + facet_wrap(~`query name`)
...
table(REF_RIBOS$Assembly)
ggplot(UBA_RIBOS, aes(x=score)) + geom_density() + facet_wrap(~`query name`)
table(UBA_RIBOS$`query name`)
UBA_RIBOS <- UBA_RIBOS[UBA_RIBOS$score >= 50,]
ggplot(UBA_RIBOS, aes(x=score)) + geom_density() + facet_wrap(~`query name`)
UBA_RIBOS$Assembly <- str_sub(UBA_RIBOS$`target name`, 1, 4)
heatmap.2(table(UBA_RIBOS$Assembly, UBA_RIBOS$`query name`))
